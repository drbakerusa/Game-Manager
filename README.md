...
Once the credentials are saved, Game Manager will begin the process of synchronizing game metadata with F95zone. This process will take a few minutes to complete (depending on your connection speed). Once the process is complete, Game Manager is ready to use!
...
1. *Refresh all metadata* This option will cause the application to refresh all metadata.
...
The Check for Updates (3) option will cause Game Manager to [refresh metadata](#refreshing-metadata) and then scan your library for updates to your games. Read more about it in the [Managing Game Updates](#managing-game-updates) section.
...
[^1]: These menu options are filtered by the "Recent Threshold" described in the [Configuration Options](#configuration-options) section.
